...
# CA certificate
#
factory.getCA().save("conf/cacert.pem").save("conf/cacert.der")

# Client certificate
client = factory.create("client")
client.save("conf/client.p12")

# Server certificate
server = factory.create("server", cn = (dns if usedns else ip), ip=ip, dns=dns)
server.save("conf/server.p12")

try:
    factory.getCA().save("conf/cacert.pem").save("conf/cacert.jks") # Used by the Database/library demo
    server.save("conf/server.jks", caalias="cacert")
    client.save("conf/client.jks", caalias="cacert")

    # Don't try to generate the BKS if the JKS generation fails
    try:
        server.save("conf/server.bks", caalias="cacert")
        client.save("conf/client.bks", caalias="cacert")
    except Exception as ex:
        for f in ["conf/server.bks", "conf/client.bks"]:
    ...
    for f in ["conf/server.jks", "conf/client.jks"]:
        if os.path.exists(f): os.remove(f)
